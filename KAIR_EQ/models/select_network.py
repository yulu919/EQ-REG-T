...
def define_G(opt):
    ...
    # SwinIR_loss
    # ----------------------------------------
    elif net_type == 'swinir_loss':
        from models.network_swinir_loss import SwinIR_loss as net
        netG = net(upscale=opt_net['upscale'],
                   in_chans=opt_net['in_chans'],
                   img_size=opt_net['img_size'],
                   window_size=opt_net['window_size'],
    ...
    elif net_type == 'swinir_eq_wo_pos':
        from models.network_swinir_eq_wo_pos import SwinIR_EQ as net
        netG = net(upscale=opt_net['upscale'],
                   in_chans=opt_net['in_chans'],
                   img_size=opt_net['img_size'],
                   window_size=opt_net['window_size'],
    ...
    # SwinIR_eq
    # ----------------------------------------
    elif net_type == 'swinir_eq':
        from models.network_swinir_eq import P4SwinIR_SR_Equivariant as net
        from models.g_selfatt.groups import SE2
        group = SE2(4)
        netG = net(
        group=group,
        upscale=opt_net['upscale'],
        in_chans=opt_net['in_chans'],
        out_chans=opt_net.get('out_chans', opt_net['in_chans']),
        embed_dim=opt_net['embed_dim'],
        depths=opt_net['depths'],
        # 你的 opt_net['num_heads'] 原来是 list (每stage一个)，等变版先用统一 heads：
        num_heads=opt_net['num_heads'] if isinstance(opt_net['num_heads'], int) else opt_net['num_heads'][0],
        window_size=opt_net['window_size'],
        img_range=opt_net['img_range'],
        attention_dropout_rate=opt_net.get('attn_drop_rate', 0.0),
        value_dropout_rate=opt_net.get('value_dropout_rate', 0.0),
        input_dropout_rate=opt_net.get('drop_rate', 0.0),
        norm_type=opt_net.get('norm_type', 'LayerNorm'),
        activation=opt_net.get('activation', 'Swish'),
        padding_mode=opt_net.get('padding_mode', 'constant'),
    )
