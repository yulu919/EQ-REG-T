...
class ModelBase():
    ...
    def model_to_device(self, network):
        ...
        if self.opt['dist']:
            ...
            # network = self.make_ddp_safe(network)
    ...
    def _set_by_name(self, root: nn.Module, name: str, value: torch.Tensor, is_param: bool):
        parts = name.split(".")
        obj = root
        for p in parts[:-1]:
            obj = getattr(obj, p)
        last = parts[-1]
        if is_param:
            setattr(obj, last, nn.Parameter(value, requires_grad=getattr(obj, last).requires_grad))
        else:
            # buffer
            setattr(obj, last, value)

    def make_ddp_safe(self, model: nn.Module):
        # fix parameters
        for name, p in list(model.named_parameters(recurse=True)):
            if p is None:
                continue
            try:
                ok = p.is_non_overlapping_and_dense()
            except Exception:
                ok = True
            if (not p.is_contiguous()) or (not ok):
                new_p = p.detach().clone().contiguous()
                self._set_by_name(model, name, new_p, is_param=True)

        # fix buffers
        for name, b in list(model.named_buffers(recurse=True)):
            if b is None:
                continue
            try:
                ok = b.is_non_overlapping_and_dense()
            except Exception:
                ok = True
            if (not b.is_contiguous()) or (not ok):
                new_b = b.detach().clone().contiguous()
                self._set_by_name(model, name, new_b, is_param=False)
