...
class ModelPlain(ModelBase):
    ...
    def __init__(self, opt):
        ...
        self.num_gpu = len(self.opt['gpu_ids'])
    ...
    def feed_data_loss(self, n, data, need_H=True):
        ...
        self.L = torch.cat((L, L_rot), dim=0)
        self.H = torch.cat((H, H_rot), dim=0)
    ...
    def optimize_parameters_loss(self, n, current_step):
        self.G_optimizer.zero_grad()
        # 确保梯度内存连续的
        ...
        ###### for EQ-REG ##############
        lamda = 1e-9  
        self.netG.module.rot_num_in(rot_num=n)
        # self.E = self.netG(self.L)
        self.netG_forward_loss()
        reg = self.fliter_reg(self.netG)
        # print("*****************reg:",reg)

        # self.netG_forward()
        G_loss = self.G_lossfn_weight * self.G_lossfn(self.E, self.H) + lamda * reg
        # print(G_loss)
        G_loss.backward()
