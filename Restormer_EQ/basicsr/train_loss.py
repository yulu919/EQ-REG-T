...
def main():
    ...
    groups = np.array([sum(iters[0:i + 1]) for i in range(0, len(iters))]) # array([300000])
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            mini_gt_size = mini_gt_sizes[bs_j] # 128
            mini_batch_size = mini_batch_sizes[bs_j] # 4 
            
            if logger_j[bs_j]:
                logger.info('\n Updating Patch_Size to {} and Batch_Size to {} \n'.format(mini_gt_size, mini_batch_size*torch.cuda.device_count())) 
                logger_j[bs_j] = False

            lq = train_data['lq'] # [32,3,128,128]
            gt = train_data['gt'] # [32,3,128,128]


            if mini_batch_size < batch_size:
                indices = random.sample(range(0, batch_size), k=mini_batch_size)
                lq = lq[indices] # [4,3,128,128]
                gt = gt[indices] # [4,3,128,128]
            ...
            model.feed_train_data_loss(n, {'lq': lq, 'gt' : gt}) # to device
            model.optimize_parameters_loss(n, current_iter)
